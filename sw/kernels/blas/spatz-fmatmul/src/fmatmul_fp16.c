...
void matmul_2xVL(__fp16 *c, const __fp16 *a, const __fp16 *b,
    ...
    while (p < p_end) {
        ...
        for (unsigned int m = m_start; m < m_end; m += 2) {
            ...
            // Loaded via flh (not a plain C load) so the scalar operand
            // keeps the raw fp16 bit pattern expected by vfmacc.vf/vfmul.vf
            // under e16 SEW; a plain C load would widen __fp16 to float.
            float t0, t1;
