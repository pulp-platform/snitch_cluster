...
int main() {
    ...
    local_A_ = local_A + nz*ny*nx;
    local_c = local_A_ + nz*ny*nx;

    // Copy data in TCDM
    if (snrt_is_dm_core()) {
        size_t size_A = nz*ny*nx * sizeof(double);
        size_t size_c = (2*r+1)*(2*r+1)*(2*r+1) * sizeof(double); 
    ...
    // Compute
    if (!snrt_is_dm_core()) {
        mcbox3d1r(r, nx, ny, nz, local_c, local_A, local_A_);
    }
    snrt_cluster_hw_barrier();

    // Copy data out of TCDM
    if (snrt_is_dm_core()) {
        size_t size_A_ = nz*ny*nx * sizeof(double);
