...
class J3D27PTDataGen(DataGen):
            ...
            for y in range(1, NY-1):
                for x in range(1, NX-1):
                    ...
                    for dz in range(-1, 2):
                        for dy in range(-1, 2):
                            for dx in range(-1, 2):
                                acc += C[((dz+1)*3*3) + ((dy+1)*3) + (dx+1)] * A[((z+dz)*NY*NX) + ((y+dy)*NX) + (x+dx)]
    ...
    def emit_header(self, **kwargs):
        ...
        G = self.golden_model(FAC, NZ, NY, NX, C, A)

        header += [format_scalar_definition('const int', 'fac', FAC)]
