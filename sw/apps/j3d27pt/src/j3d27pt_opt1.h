...
// The Kernel
static inline void j3d27pt_opt1(int fac, int nx, int ny, int nz, double* c, double* A, double* A_) {
    snrt_mcycle();
        ...
        for (int y = ly; y < ny-2; y += 2) {
            for (int x = lx; x < nx-2; x += 2) {
                ...
                asm volatile (
                    ...
                    "fsd       fa0, %[sb](%[wb])    \n"
                    : [fc]"+&f"(fact)
                    : [sx]"i"(8*sx), [sy]"i"(8*sy), [sb]"i"(8*sb), [wb]"r"(&A_[(z+1)*ny*nx + (y+1)*nx + (x+1)]), [c3]"r"(3)
