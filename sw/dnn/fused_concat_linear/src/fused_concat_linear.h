...
static inline int fused_concat_linear_baseline(fused_concat_linear_layer_t l) {
    ...
    gemm(l.dtype, 0, 0, 1, 0, snrt_cluster_num(), 1, 1, 1, 1, 1, 0, 0, m, n, k, 1.0,
...
static inline int fused_concat_linear_optimized(fused_concat_linear_layer_t l) {
    ...
    if (snrt_is_dm_core()) {
        snrt_dma_load_2d_tile(a, l.inputs[snrt_cluster_idx()],
                              0, 0, m, k, k, l.dtype);
        snrt_dma_wait_all();
    ...
    gemm(l.dtype, 0, 0, 0, 1, 1, 1, l.num_inputs, 0, 1, 1, 0, 0, m, n, concat_k, 1.0,
         a, l.weights, 0.0, l.linear_output);
