...
int main() {
    ...
    uint32_t errors = 0;
    uint32_t nb_bytes, dst_stride, src_stride, reps_2d, src_offset_2d, dst_offset_2d;
    ...
    for (int k = 0; k < NB_TRANSFERS; k ++) {
        ...
        id = snrt_dma_start_2d(main_start_addr, src_start_addr, nb_bytes, src_stride, src_stride, reps_2d);
        snrt_dma_wait(id);
        ...
        for (unsigned int rep = 0; rep < reps_2d; rep++) {
            ...
            for (unsigned int i = 0; i < nb_bytes; i++) {
                ...
                if (expected != actual) {
                    errors++;
                    printf ("ERROR: expected[%d] @%8x = %8x vs actual[%d] @%8x = %8x \n", src_offset_2d + i, &src_ptr[src_offset_2d + i], expected, src_offset_2d+i, &dst_ptr[src_offset_2d + i], actual);
                }
            }
        }

        // Launch transfer main memory -> dst
        id = snrt_dma_start_2d(dst_start_addr, main_start_addr, nb_bytes, dst_stride, src_stride, reps_2d);
        snrt_dma_wait(id);
        ...
        for (unsigned int rep = 0; rep < reps_2d; rep++) {
            ...
            for (unsigned int i = 0; i < nb_bytes; i++) {
                ...
                if (expected != actual) {
                    errors++;
                    printf ("ERROR: expected[%d] @%8x = %8x vs actual[%d] @%8x = %8x \n", src_offset_2d + i, &src_ptr[src_offset_2d + i], expected, dst_offset_2d+i, &dst_ptr[dst_offset_2d + i], actual);
                }
            }
        }
