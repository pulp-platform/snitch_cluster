...
int main() {
    if (snrt_is_compute_core() && (snrt_cluster_core_idx() == 0)) {
        ...
        // VFSHUFFLE FP32
        mask_a = 0x10; // Choose original vector
        mask_b = 0x98; // Also chose original vector, vector select only used for `vfshuffle2`
        ...
        asm volatile(
            ...
            "fmv.s.x ft3, %2\n" // mask_a = 0x10
            "fmv.s.x ft4, %3\n" // mask_b = 0x98
            "vfshuffle.s ft5, ft2, ft3\n" // ft5 = [3.14, -2.39062]
            "vfshuffle.s ft6, ft2, ft4\n" // ft6 = [3.14, -2.39062]
            "vfeq.s %4, ft5, ft2\n" //  res0 = (ft5 == ft2) = 0x3
            "vfeq.s %5, ft6, ft2\n" //  res1 = (ft6 == ft2) = 0x3
            : "+r"(i_a), "+r"(i_cn), "+r"(mask_a), "+r"(mask_b), "+r"(res0), "+r"(res1));
        errs += (res0 != 0x3);
        errs += (res1 != 0x3);

        mask_a = 0x01; // Swap vector
        mask_b = 0x89; // Also swap vector, vector select only used for `vfshuffle2`
