...
int main() {
    ...
    if (i == 2) {
        ...
        // pv.shuffle2.h
        {
            register int32_t rd asm("a3") = 0xDEADBEAF;   // rD initial (upper=0xDEAD, lower=0xBEAF)
            register int32_t rs1 asm("a4") = 0x12345678;  // rs1 (upper=0x1234, lower=0x5678)
            register int32_t rs2 asm("a5") = 0x00020001;  // control: upper→from rs1[1]=0x1234, lower→from rd[0]=0x5678
            int32_t result_rd;
        ...
        {
            int errs = 0;
            register int32_t rd asm("a3") = 0xAAAABBBB;   // rd initial = [AA][AA][BB][BB]
            register int32_t rs1 asm("a4") = 0x11223344;  // rs1        = [11][22][33][44]
            // rs2 control:
