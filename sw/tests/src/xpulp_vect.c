...
int main() {
    ...
    if (i == 2) {
        ...
        {
            register int32_t rd  asm("a3") = 0x00000000;
            register int32_t rs1 asm("a4") = 0x10203040; // [0x10,0x20,0x30,0x40]
            register int32_t rs2 asm("a5") = 0x01020304; // [0x01,0x02,0x03,0x04]
        ...
        // pv.or.h
        {
            ...
            result_rd = rd; // [0x0F0F|0x00F0=0x0FFF, 0x00F0|0x000F=0x00FF] -> 0x0FFF00FF
        ...
        // pv.xor.h
        {
            ...
            result_rd = rd; // [0xAAAA^0x0F0F=0xA5A5,0x5555^0xF0F0=0xA5A5] -> 0xA5A5A5A5
        ...
        {
            register int32_t rd asm("a3")=0;
            register int32_t rs1 asm("a4")=0x80F70810; // [0x80,0xF7,0x08,0x10]
        ...
        // pv.dotusp.sc.h (unsigned × signed scalar)
        {
            register int32_t rd asm("a3") = 0;
            register int32_t rs1 asm("a4") = 0x00030004; // [3,4] unsigned
            register int32_t rs2 asm("a5") = 0xFFFE;     // scalar = -2 signed (low 16b replicated)
        ...
        // pv.dotusp.sc.b (unsigned × signed scalar)
        {
            ...
            register int32_t rs2 asm("a5") = 0xFFFFFFFD; // scalar = -3 signed (low 8b replicated)
        ...
        {
            register int32_t rd asm("a3") = 10;                   // start with 10 to test accumulation
            register int32_t rs1 asm("a4") = 0xFFFE0003;          // [-2,3]
            register int32_t rs2 asm("a5") = 0x00040005;          // [4,5]
        ...
        // pv.sdotsp.sc.h (signed × signed scalar + accumulate)
        {
            ...
            register int32_t rs1 asm("a4") = 0x00030004;          // [3,4]
            register int32_t rs2 asm("a5") = 0xFFFD;              // scalar = -3 (low 16b replicated)
        ...
        // pv.sdotsp.b (signed × signed + accumulate)
        {
            ...
            asm volatile("pv.sdotsp.b a3, a4, a5\n"
                         ...
                         : "r"(rs1), "r"(rs2)
                         : "a3","a4","a5");
            result_rd = rd; // 5 + (1*5 + (-1)*6 + 2*7 + 3*8) = 5 + (5 -6 +14 +24) = 42
        ...
        // pv.sdotsp.sc.b (signed × signed scalar + accumulate)
        {
            ...
            asm volatile("pv.sdotsp.sc.b a3, a4, a5\n"
                         ...
                         : "r"(rs1), "r"(rs2)
                         : "a3","a4","a5");
            result_rd = rd; // -8 + (1*3 + 2*3 + 3*3 + (-3)*3) = -8 + (3 +6 +9 -9) = 1
            if (result_rd != 1) errs++;
        }
        ...
        // pv.sdotsp.sci.b (signed × signed imm + accumulate)
        {
            ...
            asm volatile("pv.sdotsp.sci.b a3, a4, 2\n"
                         ...
                         : "r"(rs1)
                         : "a3","a4");
            result_rd = rd; // 4 + (1*2 + (-1)*2 + 2*2 + 3*2) = 4 + (2 -2 +4 +6) = 14
            if (result_rd != 14) errs++;
        }
        ...
        // pv.sdotup.b (unsigned × unsigned + accumulate)
        {
            ...
            asm volatile("pv.sdotup.b a3, a4, a5\n"
                         ...
                         : "r"(rs1), "r"(rs2)
                         : "a3","a4","a5");
            result_rd = rd; // 4 + (1*5 + 2*6 + 3*7 + 4*8) = 4 + (5+12+21+32) = 74
        ...
        // pv.sdotusp.b (unsigned × signed + accumulate)
        {
            ...
            asm volatile("pv.sdotusp.b a3, a4, a5\n"
                         ...
                         : "r"(rs1), "r"(rs2)
                         : "a3","a4","a5");
            result_rd = rd; // 3 + (5*1 + 6*-1 + 7*2 + 8*3) = 3 + (5 -6 +14 +24) = 40
