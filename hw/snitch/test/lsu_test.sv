...
/// A set of testbench utilities for LSU interfaces.
package lsu_test;
  ...
  /// A driver for the LSU interface.
  class lsu_driver #(
    parameter int  AW = -1,
    parameter int  DW = -1,
  ...
  // Super classs for random lsu drivers.
  virtual class rand_lsu #(
    // LSU interface parameters
    parameter int   AW = 32,
    parameter int   DW = 32,
    ...
    function new(virtual LSU_BUS_DV #( .ADDR_WIDTH (AW), .DATA_WIDTH (DW)) bus);
      this.drv = new (bus);
    endfunction
  ...
  ) extends rand_lsu #(.AW(AW), .DW(DW), .TA(TA), .TT(TT));
