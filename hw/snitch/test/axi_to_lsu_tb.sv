...
module axi_to_lsu_tb import snitch_pkg::*; #(
  ...
  lsu_monitor_t lsu_monitor = new (slave_dv);
  // LSU Monitor.
  initial begin
    @(posedge rst_n)
    lsu_monitor.monitor();
  ...
  typedef lsu_test::rand_lsu_slave #(
    // LSU bus interface paramaters;
    .AW ( AW ),
    .DW ( DW ),
    // Stimuli application and test time
  ...
  lsu_rand_slave_t rand_lsu_slave = new (slave_dv);

  // LSU Slave.
  initial begin
    rand_lsu_slave.reset();
    @(posedge rst_n);
    rand_lsu_slave.run();
  end
