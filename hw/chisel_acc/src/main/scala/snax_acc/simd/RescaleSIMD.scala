...
class RescaleSIMD(params: RescaleSIMDParams) extends Module with RequireAsyncReset {
  ...
  for (i <- 0 until ctrl_csr_set_num) {
    ...
    ctrl_csr(i).multiplier_i := io.ctrl
      .bits(2 + ctrl_csr_set_num / packed_shift_num + i)
      .asSInt
    // ---------------------
  ...
  // generate the CSR address map for SIMD

  csrMap += s"#define SIMD_CSR0 (SUBTRACTIONS + 1)\n"
  csrMap += s"#define SIMD_CSR1 (SIMD_CSR0 + 1)\n"

  csrMap += s"#define SIMD_SHARED_BITPACKED_SHIFT (SIMD_CSR1 + 1)\n"
  csrMap += s"#define SIMD_SHARED_BITPACKED_SHIFT_NUM ${ctrl_csr_set_num / packed_shift_num}\n"

  csrMap += s"#define SIMD_SHARED_MULTIPLIER (SIMD_SHARED_BITPACKED_SHIFT + ${ctrl_csr_set_num / packed_shift_num})\n"
  csrMap += s"#define SIMD_SHARED_MULTIPLIER_NUM $ctrl_csr_set_num\n"

  csrMap += s"#define TEMPORAL_LOOP_BOUND (SIMD_SHARED_MULTIPLIER + $ctrl_csr_set_num)\n"
...
// This file is generated by RescaleSIMD module in hw/chisel_acc to map the CSR address of SIMD automatically, do not modify it manually
