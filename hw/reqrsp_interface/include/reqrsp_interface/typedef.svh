...
`define GENERIC_REQRSP_TYPEDEF_REQ_T(__req_t, __req_chan_t) \
  typedef `GENERIC_REQRSP_REQ_STRUCT(__req_chan_t) __req_t;

`define GENERIC_REQRSP_TYPEDEF_RSP_T(__rsp_t, __rsp_chan_t) \
  typedef `GENERIC_REQRSP_RSP_STRUCT(__rsp_chan_t) __rsp_t;

`define REQRSP_TYPEDEF_REQ_T(__req_t, __req_chan_t) \
  `GENERIC_REQRSP_TYPEDEF_REQ_T(__req_t, __req_chan_t)

`define REQRSP_TYPEDEF_RSP_T(__rsp_t, __rsp_chan_t) \
  `GENERIC_REQRSP_TYPEDEF_RSP_T(__rsp_t, __rsp_chan_t)
