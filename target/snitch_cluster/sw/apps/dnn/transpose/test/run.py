...
def build_sw(cfg):
    ...
    subprocess.run(['make', '-C', '../../../../../', 'sw/apps/dnn/transpose'], check=True, env=env)


# Build test specification for a specific configuration
def build_test(cfg):
    return {
        'elf': f'build/{cfg.stem}/transpose.elf',
        'name': f'transpose-{cfg.stem}',
        'cmd': [
            '../../../../../../../sw/dnn/transpose/scripts/verify.py',
            "${sim_bin}",
            "${elf}",
            '--dump-results'
        ]
