...
def fig(df):
    ...
    opt1_energy_impr = df[df['impl'] == 'baseline']['energy'].to_numpy() / df[df['impl'] == 'chaining']['energy'].to_numpy()
    opt2_energy_impr = df[df['impl'] == 'baseline']['energy'].to_numpy() / df[df['impl'] == 'chaining+']['energy'].to_numpy()
    speedup = df[df['impl'] == 'baseline']['runtime'].to_numpy() / df[df['impl'] == 'chaining+']['runtime'].to_numpy()
    max_base_fpu_util = df[df['impl'] == 'baseline']['fpu_util'].max()
    max_opt2_fpu_util = df[df['impl'] == 'chaining+']['fpu_util'].max()
    def to_percentage(val):
        return int(round((val - 1) * 100))
    return {
        'GeomeanEnergyImprovementOptI': '{}'.format(to_percentage(geometric_mean(opt1_energy_impr))),
        'GeomeanSpeedup': '{}'.format(to_percentage(geometric_mean(speedup))),
        'BaselinePeakFPUUtil': '{}'.format(int(max_base_fpu_util * 100)),
        'OptimizedPeakFPUUtil': '{}'.format(int(max_opt2_fpu_util * 100)),
        'GeomeanEnergyImprovementOptII': '{}'.format(to_percentage(geometric_mean(opt2_energy_impr))),
    }
